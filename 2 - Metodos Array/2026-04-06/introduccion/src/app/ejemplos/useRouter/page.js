...
import { useRouter } from 'next/navigation'     //  Importante que venga desde next/navigation

/*
  ¿Qué son las rutas (routes)?
  Es la dirección url que conecta al usuario con el endpoint de una página

  ¿Qué es el useRouter?
  Es un hook que nos permite manipular el url actual del usuario

  ¿Qué es el historial de navegación para el browser de internet?
    Es un array que almacena los url visitados por el usuario

  ¿Qué es el objeto router?
    Es un objeto que contiene métodos importantes para manipular el url del usuario

  ¿Cómo se usa el useRouter?

    1)  Escribimos el string "use client"

    2)  Importamos useRouter desde "next/navigation"

    3)  Creamos el objeto router

          const router = useRouter()

    4)  Utilizamos los métodos del objeto router

  Métodos del objeto router:

    router.push(URL)
      Recibe un URL y lo mete en el historial.
      Además, redirecciona al usuario.
