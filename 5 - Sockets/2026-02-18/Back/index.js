...
io.on("connection", (socket) => {
	...
	/*
		Para EMITIR eventos, el backend usa estos 3 objetos:

		socket			-	Simboliza UN cliente.

							Si se emite dentro de una suscripción de socket.on, entonces sólamente el socket
							que emitió el evento original recibirá este objeto respuesta.

		io				-	Simboliza TODOS los sockets.

		io.to(room)		-	Simboliza TODOS los sockets conectados a una "room" determinada.
	*/

	socket.on("enviarMensajeBack", (data) => {
		console.log("Se recibió el mensaje", data.mensaje)

		io.emit("enviarMensajeFront", { mensaje: data.mensaje })
	})
